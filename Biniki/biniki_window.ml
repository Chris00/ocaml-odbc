    ...
    if n = 0 then (
      ...
      | _ ->
         ...
         let win = GWindow.window
                     ~title: (Biniki_messages.logiciel^" "
                              ^ Biniki_messages.version^" : "^context#base)
                     ~width:500 ()
         in
         ...
         let itemFile = GMenu.menu_item ~label: Biniki_messages.mn1
                                        ~packing: menubar#add () in
         ...
         let itemHelp = GMenu.menu_item ~label: Biniki_messages.mn3
                                        ~packing: menubar#add () in
         ...
         let _ = itemAbout#connect#activate
                   (fun () -> Biniki_misc.message_box Biniki_messages.mn4
                                                   Biniki_messages.mAbout)
         in
         ...
         let _ =
           ...
           | Some q ->
              (* add the query box *)
              let query_box = new Biniki_query_box.box context ~query: q
                                  columns records in
